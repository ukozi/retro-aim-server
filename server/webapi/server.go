...
func NewServer(listeners []string, logger *slog.Logger, handler Handler, _ interface{}, _ *state.WebAPISessionManager) *Server {
	servers := make([]*http.Server, 0, len(listeners))

	for _, l := range listeners {
		mux := http.NewServeMux()

		// Foundation PR: only root route; feature routes added in follow-ups
